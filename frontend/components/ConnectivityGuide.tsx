...
export function ConnectivityGuide({ guide }: { guide: ConnectivityGuideResult }) {
  return (
    <section className="rounded-lg bg-white p-5 shadow-[0_18px_66px_-64px_rgba(15,23,42,0.4)] ring-1 ring-slate-200/70 sm:p-6">
      <h2 className="text-2xl font-semibold text-slate-950">Before you travel</h2>
      <p className="mt-2 text-sm leading-6 text-slate-500">Simple setup notes for before departure and when you arrive.</p>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {sections.map(([key, label, icon]) => (
          <div className="rounded-md bg-[#f4f8ff] p-4 ring-1 ring-blue-100/50" key={key}>
